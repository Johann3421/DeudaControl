...
SEED_OUT  = ROOT / "docker" / "postgres" / "seed_data.sql"

# Tables to include in seed_data.sql (schema-free, data only).
# Excludes ephemeral tables: sessions, jobs, job_batches, failed_jobs,
...
SEED_TABLES = [
    ...
    "pagos", "pagos_oc", "recibos_alquiler", "configuraciones",
]
...
def main():
    ...
    # Also write seed_data.sql (data-only, no schema, no migrations)
    seed_content = generate_seed_sql(inserts)
    with open(SEED_OUT, 'w', encoding='utf-8', newline='\n') as fh:
        fh.write(seed_content)

    print(f"✓ Written : {SEED_OUT}")
    print(f"  Size    : {SEED_OUT.stat().st_size / 1024:.1f} KB")


def generate_seed_sql(inserts: dict) -> str:
    """
    Generate a data-only SQL file for seeding (no schema, no migrations table).
    Used by entrypoint.sh when the users table is empty after a fresh migrate.
    """
    out = []
    out.append("-- ================================================================")
    out.append("-- seed_data.sql — data only, no schema, no migrations table")
    out.append("-- Auto-generated by scripts/mysql_to_postgres.py")
    out.append("-- Used by docker/entrypoint.sh when users table is empty.")
    out.append("-- ================================================================")
    out.append("")
    out.append("SET client_encoding = 'UTF8';")
    out.append("SET standard_conforming_strings = on;")
    ...
    # Reset sequences for all seeded tables so new inserts don't collide
    out.append("-- ────────── RESET SEQUENCES ─────────────────────────────────────")
    out.append("")
    for table, (col, nextval) in SEQUENCES.items():
