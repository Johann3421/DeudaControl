...
export default function UtilidadesEdit({ oc, deudas = [], empresas = [], entidades = [] }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(`/utilidades/${oc.id}`);
    };

    return (
        <Layout title="Editar OC">
            <Head title="Editar OC" />
            <div className="max-w-2xl">
                <div className="mb-6">
                    <Link href={`/utilidades/${oc.id}`} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 transition-colors">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="flex items-center gap-3 pt-2">
                            <button type="submit" disabled={processing}
                                ...
                                {processing ? 'Guardando...' : 'Actualizar OC'}
                            </button>
                            <Link href={`/utilidades/${oc.id}`} className="px-6 py-2.5 rounded-xl text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors">Cancelar</Link>
                        </div>
                    </form>
                </div>
            </div>
