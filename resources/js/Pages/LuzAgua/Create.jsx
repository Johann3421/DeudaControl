...
export default function LuzAguaCreate() {
    const { data, setData, post, processing, errors } = useForm({
        ...
        monto: '',
        estado: 'pendiente',
        mes_recibo: new Date().toISOString().substring(0, 7), // AAAA-MM
    });
    ...
    return (
        <Layout title="Registrar Recibo">
            <Head title="Registrar Recibo de Luz o Agua" />
            <div className="max-w-xl mx-auto space-y-6">
                {/* Header */}
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-slate-900">Registrar Nuevo Recibo</h2>
                        <p className="text-sm text-slate-500 mt-0.5">Ingresa los datos del recibo para el control de deudas</p>
                    </div>
                    <Link
                        href="/luz-agua"
                ...
                <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Estado Inicial</label>
                            <select
                                value={data.estado}
                    ...
                    <div className="pt-2">
                        ...
                        >
                            {processing ? 'Registrando...' : 'Registrar Recibo'}
                        </button>
                    </div>
                </form>
            </div>
